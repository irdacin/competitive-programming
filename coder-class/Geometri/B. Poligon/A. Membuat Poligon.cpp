...
struct point {
  ...
  friend bool intersect(point p, point q, point r, point s) {
    ...
    auto overlap = [](int a, int b, int cur, int d) {
      return max(min(a, b), min(cur, d)) <= min(max(a, b), max(cur, d));
    };
...
void solve() {
  ...
  auto sign = [](int x) { return x >= 0 ? x > 0 : -1; };
  sort(all(polygon), [&](auto &p, auto &q) {
    if(polygon[0].cross(p, q) == 0)
      return polygon[0].dist2(p) < polygon[0].dist2(q);
    return polygon[0].cross(p, q) > 0;
  });
  ...
  for(int i = 0; i < n; i++) {
    ...
    point r = polygon[(i + 2) % n];

    int cur = sign(p.cross(q, r));
    if(cur == 0) continue;

    if(sgn == 0) sgn = cur;
    else if(sgn != cur) return print("TIDAK");
  }

  print(sgn == 0 ? "TIDAK" : "YA");
}
