...
void solve() {
  ...
  for (int i = 0; i < n; i++) {
    ...
    if (intersect(q, r, p, p)) return print(0);
    if (q.y <= p.y && p.y < r.y && p.cross(q, r) > 0) winding++;
    else if (r.y <= p.y && p.y < q.y && p.cross(q, r) < 0) winding--;
    ans = min(ans, p.dist(q));

    if (q.dist2(r) == 0) continue;
    if (q.dot(p, r) >= 0 && q.dot(p, r) <= q.dist2(r))
      ans = min(ans, abs(p.cross(q, r) / q.dist(r)));
  }

  if (winding == 0) return print(-1);
  print(ans);
}
