...
void solve(){
    int n; cin >> n;
    vvec<int> w(n, n); cin >> w;

    vvec<int> dp(n, 1 << n, -1);
    auto func = [&](auto func, int cur, int mask) -> int {
        ...
        for(int i = 0; i < n; i++){
            if(!(mask & (1 << i))){
                res = min(res, func(func, i, mask | (1 << i)) + min(w[cur][i], w[i][cur]));
            }
        }

        if(res == INF) return res = min(w[0][cur], w[cur][0]); // balik ke awal
        return res;
    };
