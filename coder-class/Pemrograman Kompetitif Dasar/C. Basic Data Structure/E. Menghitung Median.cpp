...
void solve(){
    ...
    if(sz & 1) cout << (*a.find_by_order(sz / 2)).fi;
    else cout << ((*a.find_by_order(sz / 2)).fi + (*a.find_by_order(sz / 2 - 1)).fi) / 2 << ((*a.find_by_order(sz / 2)).fi + (*a.find_by_order(sz / 2 - 1)).fi & 1 ? ".5" : "") ; 
}
