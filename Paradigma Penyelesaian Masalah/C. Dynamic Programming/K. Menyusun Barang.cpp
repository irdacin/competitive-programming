...
void solve(){
    int n, m; cin >> n >> m;
    vec<int> weight(n); cin >> weight;

    vvec<int> dp(n, m + 1, -1);
    auto func = [&](auto func, int i, int w){
        if(w == 0) return 0LL;
        if(i >= n) return INF;
        ...
        res = INF;
        res = func(func, i + 1, w);
        if(w >= weight[i]) res = min(res, func(func, i + 1, w - weight[i]) + 1);

        return res;
    };

    int res = func(func, 0, m);
    if(res == INF) rcout(-1);
    cout << res << endl;

    for(int i = 0; i < n && res > 0; i++){
        if(m >= weight[i] && res > func(func, i + 1, m - weight[i])){
            m -= weight[i], res--;
            cout << i + 1 << " \n"[res == 0];
        }
    }
