...
void solve(){
    ...
    vvec<pii> dp(n + 1, n + 1); // {cost, cnt}
    vvec<bool> computed(n + 1, n + 1, false);
    auto func = [&](auto func, int left, int right){
        if(right - left == 1) return pii{0, 1};

        pii &ret = dp[left][right];
        if(computed[left][right]) return ret;

        int res = INF, cnt = 0;
        for(int i = left + 1; i <= right - 1; i++){
            auto [leftCost, leftCnt] = func(func, left, i);
            auto [rightCost, rightCnt] = func(func, i, right);
            int curCost = a[left] * a[i] * a[right];

            int cost = leftCost + rightCost + curCost;
            if(res > cost){
                res = cost;
                cnt = (leftCnt * rightCnt) % MOD;
            } else if(res == cost){
                (cnt += (leftCnt * rightCnt) % MOD) %= MOD;
            }
        }
