...
void solve(){
    int n, m; cin >> n >> m;
    vec<int> weight(n), value(n); 
    cin >> weight >> value;
    int maxValue = accumulate(all(value), 0);

    vvec<int> dp(n + 1, maxValue + 1, INF);
    for(int i = 0; i <= n; i++) dp[i][0] = 0;

    for(int i = 1; i <= n; i++){
        for(int v = 1; v <= maxValue; v++){
            dp[i][v] = dp[i - 1][v];
            if(v >= value[i - 1]) dp[i][v] = min(dp[i][v], dp[i - 1][v - value[i - 1]] + weight[i - 1]);
        }
    }

    for(int v = maxValue; ; v--) if(dp[n][v] <= m) rcout(v);
}
