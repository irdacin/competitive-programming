...
void solve(){
    ...
    auto queries = [&](){
        ...
        // wtf? why did I check the number with -1 ??? shitt
        vec<int> number(26, -INF);
        map<int, int> symbol;
        for(int i = 0; i < n; i++){
            if(number[s[i] - 'a'] != -INF && number[s[i] - 'a'] != a[i]) rcout("NO");
            if(symbol.find(a[i]) != symbol.end() && symbol[a[i]] != s[i] - 'a') rcout("NO");
            number[s[i] - 'a'] = a[i];
            symbol[a[i]] = s[i] - 'a';
        }
