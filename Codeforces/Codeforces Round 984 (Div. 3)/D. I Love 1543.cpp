...
void solve(){
    ...
    for(int i = 0; i < min(n, m) / 2; i++){
        string cur = "";
        for(int j = i; j < m - i; j++) cur += grid[i][j];
        for(int j = i + 1; j < n - i; j++) cur += grid[j][m - i - 1];
        for(int j = m - i - 2; j >= i; j--) cur += grid[n - i - 1][j];
        for(int j = n - i - 2; j > i; j--) cur += grid[j][i];

        for(int i = 0; i < len(cur); i++){
            int one = i;
            int two = (i + 1) % len(cur);
            int three = (i + 2) % len(cur);
            int four = (i + 3) % len(cur);

            if(cur[one] == '1' && cur[two] == '5' && cur[three] == '4' && cur[four] == '3') ans++;
        }
    }
