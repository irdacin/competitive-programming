...
void solve() {
	...
	vec<int> neg, pos, zero;
	for(auto x : a) {
		if(x < 0) neg.eb(x);
		else if(x > 0) pos.eb(x);
		else zero.eb(x);
	}
	if(pos.empty()) pos.eb(neg.back()), neg.pop_back(), pos.eb(neg.back()), neg.pop_back();
	if(len(neg) & 1 ^ 1) zero.eb(neg.back()), neg.pop_back();

	prinsp(len(neg));
	for(auto x : neg) prinsp(x);
	print();

	prinsp(len(pos));
	for(auto x : pos) prinsp(x);
	print();

	prinsp(len(zero));
	for(auto x : zero) prinsp(x);
	print();
}
