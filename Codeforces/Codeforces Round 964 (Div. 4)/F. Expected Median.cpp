...
int binpow(int a, int k){
    if(k == 0) return 1;
    return binpow(a * a % MOD, k >> 1) * (k & 1 ? a : 1) % MOD;
}
...
void solve(){
    ...
    int zero = 0, one = 0;
    for(auto i : a) zero += (i == 0), one += (i == 1);
    
    int ans = 0;
    for(int cntZero = 0; cntZero <= zero; cntZero++){
        int cntOne = k - cntZero;

        if(cntOne > cntZero && cntOne <= one){
            // cout << cntOne << ' ' << cntZero << endl;

            ans += comb(one, cntOne) * comb(zero, cntZero) % MOD;
            ans %= MOD;
        }
    }
