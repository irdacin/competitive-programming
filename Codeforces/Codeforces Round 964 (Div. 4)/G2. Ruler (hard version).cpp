...
void solve(){
    int ans = 2;
    for(int left = 2, right = 999; left <= right;){

        // ternary search
        int mid1 = left + (right - left) / 3;
        int mid2 = right - (right - left) / 3;

        cout << '?' << ' ' << mid1 << ' ' << mid2 << endl << flush;
        int x; cin >> x;

        if(mid1 * mid2 == x) ans = mid2 + 1, left = mid2 + 1;
        else if((mid1 + 1) * (mid2 + 1) == x) ans = mid1, right = mid1 - 1;
        else ans = mid1 + 1, left = mid1 + 1, right = mid2 - 1;
    }
