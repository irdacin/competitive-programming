...
void solve(){
    ...
    map<pii, int> dp;
    auto func = [&](auto func, int i, int div){
        if(div >= INF) return 0LL;
        if(i == n) return 0LL;
        
        if(dp.find({i, div}) != dp.end()) return dp[{i, div}];

        int &res = dp[{i, div}];
        res = max(func(func, i + 1, div) + a[i] / div - k, func(func, i + 1, div * 2) + a[i] / (div * 2));
        return res;
    };
