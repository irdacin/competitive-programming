...
void solve(){
    ...
    for(int i = 0; i < n; i++){
        for(int j = 0; j < n; j++){
            int zero = 0, one = 0;
            // 0 derajat
            zero += grid[i][j] == '0', one += grid[i][j] == '1';

            // 90 derajat
            zero += grid[n - j - 1][i] == '0', one += grid[n - j - 1][i] == '1';

            // 180 derajat
            zero += grid[j][n - i - 1] == '0', one += grid[j][n - i - 1] == '1';

            // 270 derajat
            zero += grid[n - i - 1][n - j - 1] == '0', one += grid[n - i - 1][n - j - 1] == '1';
