...
struct SegmentTree {
  struct node {
    ...
    int ans;
    int zero, one;
    node() : ans(0), zero(0), one(0) {}
    node(int v) : val(v), ans(1), zero(v == 0), one(v == 1) {}

    friend node merge(const node& left, const node& right) {
      node res;
      int take = min(left.zero , right.one);
      res.ans = left.ans + right.ans - take * 2;
      res.zero = left.zero + right.zero - take;
      res.one = left.one + right.one - take;
      return res;
    }
  };
  ...
  SegmentTree(const string& v) : SegmentTree(len(v)) {
    for(int i = 0; i < n; i++) tree[i + n] = node(v[i] - '0');
...
void solve() {
  ...
  for(int k; q--;) {
    cin >> k;
    if(int x, l, r; k == 1) cin >> x, seg.update(x);
    else cin >> l >> r, print(seg.query(l, r));
  }
}
