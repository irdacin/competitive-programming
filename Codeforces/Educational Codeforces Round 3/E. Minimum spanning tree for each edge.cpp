...
void solve() {
  int n, m; cin >> n >> m;
  vec<iii> edges(m);
  for(int i = 0; auto &[w, u, v, id] : edges) cin >> u >> v >> w, u--, v--, id = i++;
  sort(all(edges));
  ...
  for(auto [w, u, v, id] : edges) {
    if(ans[id] == -1) {
      int par = tree.lca(u, v);
      int cur = mst + w - max(tree.kth_ancestor(u, tree.depth[u] - tree.depth[par]), tree.kth_ancestor(v, tree.depth[v] - tree.depth[par]));
      ans[id] = cur;
    }
  }
