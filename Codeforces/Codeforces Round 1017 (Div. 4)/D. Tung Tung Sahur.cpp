...
void solve(){
  ...
  for(; i < len(s) && j < len(p); i++){
    if(s[i] != p[j]) return print("NO");
    int cnt1 = 0, cnt2 = 0;
    for(; i < len(s) && s[i] == p[j]; i++, cnt1++); i--;
    for(; j < len(p) && s[i] == p[j]; j++, cnt2++);

    if(cnt1 < cnt2 || cnt1 > cnt2 * 2) return print("NO");
  }
  if(i < len(s) || j < len(p)) return print("NO");
  print("YES");
}
