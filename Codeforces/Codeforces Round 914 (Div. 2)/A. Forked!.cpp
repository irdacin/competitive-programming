...
void solve() {
  ...
  int dify = abs(yq - yk);
  prind(difx, dify);
  if(difx == 0 && dify == a * 2) return print(2);
  if(dify == 0 && difx == b * 2) return print(2);

  int ans = 0;
  set<pii> st;
  for(auto [kingx, kingy] : vec<pii>{{a, b}, {-a, b}, {a, -b}, {-a, -b}, {b, a}, {b, -a}, {-b, a}, {-b, -a}}) {
    kingx += xk, kingy += yk;
    for(auto [queenx, queeny] : vec<pii>{{a, b}, {-a, b}, {a, -b}, {-a, -b}, {b, a}, {b, -a}, {-b, a}, {-b, -a}}) {
      queenx += xq, queeny += yq;
      if(kingx == queenx && kingy == queeny) st.emp(kingx, kingy);
    }
  }
