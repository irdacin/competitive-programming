...
void solve() {
  int n; cin >> n;
  vec<int> a(n), b(n + 1); cin >> a >> b;

  vvec<int> dp(n, 2, -1);
  auto func = [&](auto func, int i, bool take) {
    if(i == n) return take ? 0LL : INF;

    int& res = dp[i][take];
    if(res != -1) return res;

    res = func(func, i + 1, take) + abs(a[i] - b[i]);
    if(!take) res = min(res, func(func, i + 1, 1) + abs(a[i] - b[i]) + (b[n] < min(a[i], b[i]) || b[n] > max(a[i], b[i]) ? min(abs(a[i] - b[n]), abs(b[i] - b[n])) : 0) + 1);
    return res;
  };
  print(func(func, 0, 0));
}
