...
void solve(){
    ...
    vec<char> digits, letters;
    for(bool num = s[0] >= '0' && s[0] <= '9'; auto c : s){
        num &= c >= '0' && c <= '9';
        if(!num && c >= '0' && c <= '9') rcout("NO");

        if(c >= '0' && c <= '9') digits.eb(c);
        else letters.eb(c);
    }

    if(is_sorted(all(digits)) && is_sorted(all(letters))) cout << "YES";
    else cout << "NO";
}
