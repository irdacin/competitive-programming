...
void solve(){
    int h, n; cin >> h >> n;
    vec<pii> v(n);
    for(auto &[c, a] : v) cin >> a;
    for(auto &[c, a] : v) cin >> c;
    sort(all(v));

    priority_queue<array<int, 3>, vec<array<int, 3>>, greater<>> pq;
    for(auto [c, a] : v) pq.push({0, a, c});

    int ans = 1;
    for(int last = 0; !pq.empty() && h > 0; ){
        auto [cur, a, c] = pq.top(); pq.pop();
        // cout << c << ' ' << a << ' ' << cur << endl;
        h -= a;
        // cout << "h => " << h << endl;
        ans += cur - last;
        last = cur;
        if(h > 0) pq.push({cur + c, a, c});
    }
