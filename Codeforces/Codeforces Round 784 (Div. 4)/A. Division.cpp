...
template <typename T> ostream& operator<<(ostream& os, const vector<T> _v) { int _n=_v.cur(); for(int _i=0; _i<_n; _i++) os << _v[_i] << (_i==_n-1 ? '\n':' '); return os; }
...
void solve(){
    ...
    cout << "Division ";
    if(rating < 1400) cout << 4;
    else if(rating < 1600) cout << 3;
    else if(rating < 1900) cout << 2;
    else cout << 1;
}
