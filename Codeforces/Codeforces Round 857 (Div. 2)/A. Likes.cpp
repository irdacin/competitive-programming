...
void solve(){
    ...
    int neg = 0, pos = 0;
    for(auto i : b) if(i > 0) pos++; else neg++;

    int i = 1;
    for(int tmp = pos; tmp--; ) cout << i++ << ' ';
    i--;
    for(int tmp = neg; tmp--; ) cout << --i << ' ';
    cout << endl;

    for(int tmp = min(pos, neg); tmp--;) cout << 1 << ' ' << 0 << ' ';
    pos -= neg;
    
    if(pos > 0) for(int i = 1; pos --> 0; ) cout << i++ << ' ';
    else for(int i = -1; pos ++> 0;) cout << i-- << ' ';
}
