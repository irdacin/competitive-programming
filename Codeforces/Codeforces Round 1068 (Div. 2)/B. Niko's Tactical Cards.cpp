...
void solve() {
  ...
  for(int i = 1; i <= n; i++) {
    chmin(dp[i][0], dp[i - 1][0] - a[i - 1]);
    chmin(dp[i][0], dp[i - 1][1] - a[i - 1]);
    chmin(dp[i][0], b[i - 1] - dp[i - 1][0]);
    chmin(dp[i][0], b[i - 1] - dp[i - 1][1]);

    chmax(dp[i][1], dp[i - 1][0] - a[i - 1]);
    chmax(dp[i][1], dp[i - 1][1] - a[i - 1]);
    chmax(dp[i][1], b[i - 1] - dp[i - 1][0]);
    chmax(dp[i][1], b[i - 1] - dp[i - 1][1]);
  }
  print(dp[n][1]);
}
