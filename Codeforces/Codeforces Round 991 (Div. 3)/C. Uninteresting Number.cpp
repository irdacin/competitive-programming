...
void solve(){
    ...
    int sum = 0;
    int two = 0, three = 0;
    for(auto c : s) sum += c - '0', two += c == '2', three += c == '3';

    auto func = [&](auto func, int sum, int two, int three, int cntTwo = 0, int cntThree = 0) -> bool {
        if(sum % 9 == 0) return true;
        if(two > 0 && cntTwo < 9 && func(func, sum + 2, two - 1, three, cntTwo + 1, cntThree)) return true; 
        if(three > 0 && cntThree < 3 && func(func, sum + 6, two, three - 1, cntTwo, cntThree + 1)) return true;
        return false;
    };

    cout << (func(func, sum, two, three) ? "YES" : "NO");
}
