...
void solve(){
    ...
    // s1 must be ... and s2 must be x.x or the reverse

    int ans = 0;
    for(int i = 1; i < n - 1; i++){
        string curS1 = "", curS2 = "";
        for(auto idx : {i - 1, i, i + 1}) curS1 += s1[idx], curS2 += s2[idx];
        // cout << curS1 << ' ' << curS2 << endl;

        if(curS1 == "..." && curS2 == "x.x" || curS1 == "x.x" && curS2 == "...") ans++;
    }
