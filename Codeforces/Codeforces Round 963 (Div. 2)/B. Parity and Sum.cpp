...
void solve(){
    int n; cin >> n;
    priority_queue<int> odd; // actually ni ga kepake krn perlu maxnya doang
    // priority_queue<int, vec<int>, greater<>> even;
    multiset<int> even; // mirip min heap, tapi bisa ascess depan belakang
    ...
    while(!odd.empty() && !even.empty()){
        int curOdd = odd.top(), curEven = *even.begin();
        if(curOdd > curEven) even.erase(even.begin());
        else curEven = *even.rbegin(); // greedily take the large element

        int val = curOdd + curEven;
        // cout << curOdd << ' ' << curEven << ' ' << val << endl;
        odd.emplace(val);
        ans++;
    }
    cout << ans;
