...
void solve(){
  ...
  deque<iii> q;
  vvvec<int> dist(n, m, 4, INF);
  dist[0][0][0] = 0;
  q.eb(0, 0, 0);

  while(!q.empty()){
    auto [row, col, cur] = q.front(); q.pop_front();
    if(row + 1 == n && col + 1 == m && cur == 0) return print(dist[row][col][cur]);

    auto [x, y] = dir[cur];
    x += row, y += col;
    if(x >= 0 && x < n && y >= 0 && y < m && dist[x][y][cur] > dist[row][col][cur]){
      dist[x][y][cur] = dist[row][col][cur];
      q.emplace_front(x, y, cur);
    }

    if(grid[row][col] == '#'){
      for(int id = 0; id < 4; id++){
        if(dist[row][col][id] > dist[row][col][cur] + 1){
          dist[row][col][id] = dist[row][col][cur] + 1;
          q.eb(row, col, id);
        }
      }
    }
