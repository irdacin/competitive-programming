...
void solve() {
  string s; cin >> s;
  // if(len(s) & 1) return print(0);
  // for(char prev = s[0]; auto c : s) if(c != prev) return print(0);

  __int128_t n = stoll(s);
  vec<int> ans;

  for(__int128_t y = 10; y <= n; y *= 10) {
    if(n % (y + 1) == 0) ans.eb(n / (y + 1));
  }

  sort(all(ans));
  print(len(ans));
  for(auto c : ans) prinsp(c); print();
} 
