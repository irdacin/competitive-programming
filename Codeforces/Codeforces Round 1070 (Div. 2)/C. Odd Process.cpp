...
void solve() {
  int n; cin >> n;
  vec<int> odd, even;
  for(int a, i = 0; i < n; i++) cin >> a, (a & 1 ? odd : even).eb(a);
  sort(all(odd), greater<>()), sort(all(even), greater<>());

  vec<int> ans(n);
  if(empty(odd)) return prinv(ans);
  if(empty(even)) {
  ...
  ans = vec<int>(n, odd[0]);
  for(int i = 1; i < len(even); i++) even[i] += even[i - 1];

  for(int i = 0; i < n; i++) {
    int e = min(len(even), i);
    int o = i - e + 1;
    if(o % 2 == 0) o++, e--;
    prind(e, o);


    if(o > len(odd)) ans[i] = 0;
    else if(e > 0)
      ans[i] += even[e - 1];
  }
