...
void solve() {
  ...
  int pos = run(
    for(int i = n - 1; i >= 0; i--) if(a[i] == -1) return i;
    return -1LL;
  );
  ...
  // int ans = 0, need = 0;
  // for(int i = 0; i < n - 1; i++) {
  //   if(a[i] == -1 && i != pos) a[i] = 0;
  //   if(a[i + 1] == -1 && i + 1 != pos) a[i + 1] = 0;
  //   if(a[i] != -1 && a[i + 1] != -1) ans += a[i + 1] - a[i];
  //   // prind(ans);
  // }

  // prind(ans);
  // if(pos > 0) ans -= a[pos - 1];
  // if(pos < n - 1) ans += a[pos + 1];
  // prind(ans);
