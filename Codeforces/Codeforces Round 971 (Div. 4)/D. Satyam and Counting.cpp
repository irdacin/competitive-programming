...
void solve(){
    ...
    for(int x, y, i = 0; i < n; i++){
        ...
        // garis vertical can make all point to right triangle
        if(points.find({x, y ^ 1}) != points.end()) ans += n - 2;
        points.emplace(x, y);
    }

    // pythagoras rule
    // (akar 2)^2 + (akar 2)^2 = 2^2
    for(auto [x, y] : points){
        if(points.find({x - 1, y ^ 1}) != points.end() && points.find({x + 1, y ^ 1}) != points.end()) ans++;
    }
