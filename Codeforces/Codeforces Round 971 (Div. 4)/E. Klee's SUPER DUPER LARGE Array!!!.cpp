...
void solve(){
    ...
    for(int left = 1, right = n; left <= right;){
        ...
        int sumLeft = abs(mid * (2 * k + mid - 1) - sum);
        int sumRight = abs((mid + 1) * (2 * k + (mid + 1) - 1) - sum);

        if(sumLeft > sumRight) left = mid + 1;
        else num = mid, right = mid - 1;
    }

    cout << abs(num * (2 * k + num - 1) - sum);
}
