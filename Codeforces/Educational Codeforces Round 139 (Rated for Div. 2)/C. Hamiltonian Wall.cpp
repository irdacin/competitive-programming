...
void solve() {
  int m; cin >> m;
  vvec<char> grid(2, m); cin >> grid;
  int b = count(all(grid[0]), 'B') + count(all(grid[1]), 'B');

  for(int r = 0; r < 2; r++) {
    if(grid[r][0] != 'B') continue;
    ...
    while(!q.empty()) {
      ...
      for(auto [x, y] : vec<pii>{{row ^ 1, col}, {row, col + 1}}) {
        if(x >= 0 && x < 2 && y >= 0 && y < m && grid[x][y] == 'B' && !vis[x][y]) {
          vis[x][y] = true;
          q.emp(x, y);
          break;
        }
    ...
    // print(vis);
    if(count(all(vis[0]), 1) + count(all(vis[1]), 1) == b) return print("YES");
