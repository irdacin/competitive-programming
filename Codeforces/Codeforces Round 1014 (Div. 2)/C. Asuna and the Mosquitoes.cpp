...
void solve(){
  ...
  vec<int> v[2];
  for(int a; n--;) cin >> a, v[a & 1].eb(a);

  for(int i = 0; i < 2; i++) sort(all(v[i]), greater<>());

  if(v[1].empty()) return print(v[0].front());

  int ans = v[1].front();
  
  bool can = false;
  for(auto i : v[0]) can = true, ans += i;
  if(can) for(auto i : v[1] | views::drop(1)) ans += i - 1;
  print(ans);
}
