...
void solve() {
  ...
  for(int i = 1; i < n; i++) {
    if(abs(a[i] - a[i - 1]) <= 1) return print(0);
  }

  // if(n == 2) return print(-1);
  for(int i = 1; i < n - 1; i++) {
    if(abs(min(a[i], a[i + 1]) - a[i - 1]) <= 1 || abs(max(a[i], a[i + 1]) - a[i - 1]) <= 1) return print(1);
    if(abs(min(a[i], a[i - 1]) - a[i + 1]) <= 1 || abs(max(a[i], a[i - 1]) - a[i + 1]) <= 1) return print(1);
    if(min(a[i], a[i + 1]) <= a[i - 1] && a[i - 1] <= max(a[i], a[i + 1])) return print(1);
    if(min(a[i], a[i - 1]) <= a[i + 1] && a[i + 1] <= max(a[i], a[i - 1])) return print(1);
  }
  print(-1);
}
