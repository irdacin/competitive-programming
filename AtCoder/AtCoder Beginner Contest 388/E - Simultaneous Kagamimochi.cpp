...
void solve(){
  ...
  int ans = 0;
  for(int i = 0, last = n / 2 - 1; i < n; i++){
    if(used[i]) continue;

    int pos = n;
    for(int left = last + 1, right = n - 1; left <= right;){
      int mid = (left + right) / 2;
      if(a[mid] < a[i] * 2) left = mid + 1;
      else pos = mid, right = mid - 1;
    }
    
    // for(; pos < n && used[pos]; pos++);
    if(pos < n) used[i] = used[pos] = true, ans++, last = pos;
  }
