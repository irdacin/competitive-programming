...
void solve(){
    int n; cin >> n;
    vec<double> p(n); cin >> p;

    vvec<double> dp(n, n, -1);
    auto func = [&](auto func, int i, int cnt){
        if(i == n){
            if(cnt * 2 < n) return 0.0;
            return 1.0;
        }
        ...
        res = 0;
        res += func(func, i + 1, cnt + 1) * p[i];
        res += func(func, i + 1, cnt) * (1 - p[i]);

        return res;
    };

    cout << setprecision(9) << fixed << func(func, 0, 0);
}
