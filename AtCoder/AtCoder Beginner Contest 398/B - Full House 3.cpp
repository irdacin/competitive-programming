...
void solve(){
  vec<int> freq(14);
  for(int i = 0, a; i < 7; i++) cin >> a, freq[a]++;

  bool two = false, three = false, other = false;
  for(auto i : freq) two |= i == 2, three |= i == 3, other |= i > 3;
  print(two && three || two && other || three && other ? "Yes" : "No");
}
