...
void solve(){
  vec<int> cnt(14, 0);
  for(int i = 0, a; i < 4; i++) cin >> a, cnt[a]++;

  bool three = false, one = false;
  bool two1 = false, two2 = false;
  for(auto i : cnt){
    if(i == 3) three = true;
    else if(i == 1) one = true;
    else if(i == 2 && !two1) two1 = true;
    else if(i == 2 && !two2) two2 = true;
  }

  if(three && one) print("Yes");
  else if(two1 && two2) print("Yes");
  else print("No");
}
