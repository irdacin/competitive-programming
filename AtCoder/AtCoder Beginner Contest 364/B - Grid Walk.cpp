...
template <typename T, typename U> istream& operator>>(istream& is, const pair<T, U> _p) { is >> _p.first >> _p.second; return is; }
...
void solve(){
    int h, w; cin >> h >> w;
    pii start; cin >> start.fi >> start.se, start.fi--, start.se--;
    vvec<char> grid(h, w); cin >> grid;
    string path; cin >> path;

    for(auto c : path){
        pii nxt;
        if(c == 'U') nxt = {start.fi - 1, start.se};
        else if(c == 'L') nxt = {start.fi, start.se - 1};
        else if(c == 'R') nxt = {start.fi, start.se + 1};
        else nxt = {start.fi + 1, start.se};

        auto [row, col] = nxt;
        if(row < 0 || row >= h || col < 0 || col >= w || grid[row][col] == '#') continue;
        start = nxt;
        // cout << start << endl;
    }
    start.fi++, start.se++;
