...
void solve() {
  ...
  vec<int> pref(n + 1), pref1(n + 1), pref2(n + 1);
  for(int i = 1; i <= n; i++) {
    pref[i] = pref[i - 1] + a[i - 1];
    pref1[i] = pref1[i - 1] + a[i - 1] * i;
    pref2[i] = pref2[i - 1] + a[i - 1] * i * i;
  }
  ...
  for(int l, r; q--;) {
    ...
    */
    ...
    int fir = (pref[r] - pref[l - 1]) * (r - l - r * l + 1);
    int sec = (pref1[r] - pref1[l - 1]) * (r + l);
    int th = (pref2[r] - pref2[l - 1]) * -1;
    print(fir + sec + th);
