...
void solve(){
  ...
  vvec<int> dp(len(s), 2, -1);
  auto func = [&](auto func, int i, bool isUpper){
    if(i == len(s)) return 0LL;

    int &res = dp[i][isUpper];
    if(res != -1) return res;
    ...
    if(s[i] == 'A'){
      if(!isUpper){
        res = min(res, func(func, i + 1, isUpper ^ 1) + x + z);
        res = min(res, func(func, i + 1, isUpper) + y);
      } else {
        res = min(res, func(func, i + 1, isUpper) + x);
        res = min(res, func(func, i + 1, isUpper ^ 1) + y + z);
      }
    } else {
      if(isUpper){
        res = min(res, func(func, i + 1, isUpper ^ 1) + x + z);
