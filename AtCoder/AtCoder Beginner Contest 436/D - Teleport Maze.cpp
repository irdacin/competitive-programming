...
void solve() {
  ...
  for(int i = 0; i < h; i++) {
    for(int j = 0; j < w; j++) 
      if(grid[i][j] >= 'a' && grid[i][j] <= 'z') pos[grid[i][j] - 'a'].eb(i, j);
  }

  // print(pos[1]);
  // for(auto [x, y] : pos[0]) print(x, y);
  ...
  while(!empty(q)) {
    ...
    if(row == h - 1 && col == w - 1) return print(dist[row][col]);

    if(grid[row][col] >= 'a' && grid[row][col] <= 'z' && !used[grid[row][col] - 'a']) {
      used[grid[row][col] - 'a'] = true;
      for(auto [x, y] : pos[grid[row][col] - 'a']) {
    ...
    for(auto [x, y] : vec<pii>{{0, 1}, {1, 0}, {0, -1}, {-1, 0}}) {
      x += row, y += col;
      if(x >= 0 && x < h && y >= 0 && y < w && grid[x][y] != '#' && dist[x][y] > dist[row][col] + 1) {
        dist[x][y] = dist[row][col] + 1;
        q.emp(x, y);
      }
    }
