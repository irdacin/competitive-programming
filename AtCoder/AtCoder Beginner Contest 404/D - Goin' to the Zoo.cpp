...
void solve() {
  ...
  for(int k, i = 0; i < m; i++) {
    ...
    for(int a; k--;) cin >> a, zoo[--a].eb(i);
  }

  vec<int> vis(n);
  auto func = [&](auto func, int id) {
    if(id == n) {
      for(int j = 0; j < m; j++) if(animals[j] < 2) return INF;
      return 0LL;
    }

    int res = func(func, id + 1);

    if(vis[id] < 2) {
      vis[id]++;
      for(auto j : zoo[id]) animals[j]++;
      res = min(res, func(func, id) + c[id]);
      for(auto j : zoo[id]) animals[j]--;
      vis[id]--;
    }
