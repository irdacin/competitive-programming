...
void solve() {
  ...
  for(int i = 1; i <= n; i++) {
    int r = -1;
    for(int left = 1, right = i; left <= right;) {
      int mid = (left + right) / 2;
      if(pref[i][0] - pref[mid - 1][0] >= a) r = mid, left = mid + 1;
      else right = mid - 1;
    }

    int l = r + 1;
    for(int left = 1, right = r; left <= right;) {
      int mid = (left + right) / 2;
      if(pref[i][1] - pref[mid - 1][1] < b) l = mid, right = mid - 1;
      else left = mid + 1;
    }
