...
void solve(){
  int n, k; cin >> n >> k;
  vvec<int> v(2, n); cin >> v;

  vvec<int> dp(n, 2, -1); // 0 for A, 1 for B
  auto func = [&](auto func, int i, bool last) -> bool {
    if(i == n) return true;
    ...
    res = false;
    res |= abs(v[last][i - 1] - v[last][i]) <= k && func(func, i + 1, last);
    res |= abs(v[last][i - 1] - v[last ^ 1][i]) <= k && func(func, i + 1, last ^ 1);
    return res;
  };

  print(func(func, 1, 0) || func(func, 1, 1) ? "Yes" : "No");
}
