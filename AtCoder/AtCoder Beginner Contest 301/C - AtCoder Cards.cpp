...
void solve(){
  ...
  vec<int> freq(26);
  for(auto c : s) if(c == '@') other1++; else freq[c - 'a']++;
  for(auto c : t) if(c == '@') other2++; else freq[c - 'a']--;

  string atcoder = "atcoder";
  for(int i = 0; i < 26; i++){
    char c = (char) i + 'a';
    // print(c, freq[i], other1, other2);

    if(freq[i] > 0 && atcoder.find(c) != string::npos && other2 >= freq[i]) other2 -= freq[i];
    else if(freq[i] < 0 && atcoder.find(c) != string::npos && other1 >= freq[i]) other1 += freq[i];
    else if(freq[i] != 0) return print("No");
  }

  print(other1 == other2 ? "Yes" : "No");
}
