...
void solve() {
  int n; cin >> n;
  vec<iii> sky(n);
  for(auto &[u, d, l, r] : sky) cin >> u >> d >> l >> r;

  vvec<int> dif(2005, 2005);
  for(auto [u, d, l, r] : sky) {
    for(int i = u; i <= d; i++) dif[i][l]++, dif[i][r + 1]--;
  }

  int ans = 0;

  vvec<int> pref(2005, 2005);
  for(int i = 1; i <= 2000; i++) {
    int cur = 0;
    for(int j = 1; j <= 2000; j++) {
      cur += dif[i][j];
      ans += cur == 0;
      pref[i][j] = (cur == 1) + pref[i - 1][j] + pref[i][j - 1] - pref[i - 1][j - 1];
    }
  }
  ...
  for(auto [u, d, l, r] : sky) {
    // print(pref[d][r] - pref[d][l - 1] - pref[u - 1][r] + pref[u - 1][l - 1]);
    print(ans + pref[d][r] - pref[d][l - 1] - pref[u - 1][r] + pref[u - 1][l - 1]);
  }
}
