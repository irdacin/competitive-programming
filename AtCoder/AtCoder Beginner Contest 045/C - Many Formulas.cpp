...
void solve() {
  string s; cin >> s;
  auto func = [&](auto func, int i, int num, int sum) {
    if(i == len(s)) return sum + num;
    int res = func(func, i + 1, num * 10 + (s[i] - '0'), sum);
    prind(res, num);
    res += func(func, i + 1, 0, sum + num) ;
    return res;
  };
