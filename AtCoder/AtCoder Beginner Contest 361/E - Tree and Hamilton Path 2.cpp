...
void solve(){
    int n; cin >> n;
    vvec<pii> edge(n); int sum = 0;
    for(int a, b, c; n --> 1;) 
        cin >> a >> b >> c, --a, --b, edge[a].eb(b, c), edge[b].eb(a, c), sum += c * 2;

    auto dfs = [&](auto dfs, int cur, int prev, int dis) -> pii {
        int mxDis = dis, farthest = cur;
        for(auto [nxt, c] : edge[cur]){
            if(nxt != prev){
                auto [d, x] = dfs(dfs, nxt, cur, dis + c);
                if(d > mxDis){
                    mxDis = d;
                    farthest = x;
                }
        ...
        return {mxDis, farthest};
    };

    auto [d, farthest] = dfs(dfs, 0, -1, 0);
    auto [dis, f] =  dfs(dfs, farthest, -1, 0);
    cout << sum - dis;
}
