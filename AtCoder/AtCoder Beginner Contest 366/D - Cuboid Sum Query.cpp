...
void solve(){
    int n; cin >> n;
    vvvec<int> a(n, n, n); cin >> a;

    vvvec<int> pref(n + 1, n + 1, n + 1);
    for(int i = 1; i <= n; i++){
        for(int j = 1; j <= n; j++){
            for(int k = 1; k <= n; k++){
    ...
    for(pii x, y, z; q--;){
        ...
        int ans = pref[x.se][y.se][z.se];
        
        ans -= pref[x.fi - 1][y.se][z.se];
        ans -= pref[x.se][y.fi - 1][z.se];
        ans -= pref[x.se][y.se][z.fi - 1];

        ans += pref[x.fi - 1][y.fi - 1][z.se];
        ans += pref[x.fi - 1][y.se][z.fi - 1];
        ans += pref[x.se][y.fi - 1][z.fi - 1];

        ans -= pref[x.fi - 1][y.fi - 1][z.fi - 1];
