...
void solve(){
  ...
  int ans = len(child);
  for(int i = 0; i < len(adult); i++){
    int cntChild = 0;
    for(int left = 0, right = len(child) - 1; left <= right;){
      int mid = (left + right) / 2;
      if(child[mid] < adult[i]) cntChild = mid + 1, left = mid + 1;
      else right = mid - 1;
    }
    
    int cntAdult = len(adult) - i;
    ans = max(ans, cntChild + cntAdult);
  }
  print(ans);
}
