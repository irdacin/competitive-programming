  ...
  for(int i = 0; i < h; i++){
    for(int j = 0; j < w; j++){
      if(grid[i][j] == 'S'){
        ...
        while(!q.empty()){
          auto [row, col, cur] = q.front(); q.pop();
          if(grid[row][col] == 'G') return print(dist[row][col][cur]);

          for(auto [x, y] : dir[cur ^ 1]){
            x += row, y += col;
            if(x >= 0 && x < h && y >= 0 && y < w && grid[x][y] != '#' && dist[x][y][cur ^ 1] > dist[row][col][cur] + 1){
              dist[x][y][cur ^ 1] = dist[row][col][cur] + 1;
              q.emplace(x, y, cur ^ 1);
            }
          }
        }
