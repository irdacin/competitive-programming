...
void solve(){
  int n; cin >> n;
  vec<iii> people;
  for(int a, b, i = 1; i <= n; i++) cin >> a >> b, b += a, people.eb(a, b, i);

  sort(all(people), [](auto a, auto b){
    auto [nume1, deno1, id1] = a;
    auto [nume2, deno2, id2] = b;
    if(nume1 * deno2 == nume2 * deno1) return id1 < id2;
    return nume1 * deno2 > nume2 * deno1;
  });

  for(auto [nume, deno, id] : people) printsp(id);
}
