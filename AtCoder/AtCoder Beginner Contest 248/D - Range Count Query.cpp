...
void solve(){
  ...
  for(int l, r, x; q--;){
    ...
    int posLeft = len(idx[x]);
    for(int left = 0, right = len(idx[x]) - 1; left <= right;){
      int mid = (left + right) / 2;

      if(idx[x][mid] >= l) posLeft = mid, right = mid - 1;
      else left = mid + 1;
    }
    ...
    for(int left = 0, right = len(idx[x]) - 1; left <= right;){
      ...
      if(idx[x][mid] <= r) posRight = mid, left = mid + 1;
      else right = mid - 1;
    }

    print(posRight - posLeft + 1);
  }
}
