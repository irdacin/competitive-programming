...
void solve() {
  ...
  vec<iii> a(n);
  int totw = 0;
  for(auto &[w, h, b] : a) cin >> w >> h >> b, totw += w;

  vvec<int> dp(n, totw, -1);
  auto func = [&](auto func, int i, int wh) {
    if(i == n) return wh <= totw - wh ? 0LL : -INF;

    int &res= dp[i][wh];
    if(res != -1) return res;

    auto [w, h, b] = a[i];
    return res = max(func(func, i + 1, wh) + b, func(func, i + 1, wh + w) + h);
  };
