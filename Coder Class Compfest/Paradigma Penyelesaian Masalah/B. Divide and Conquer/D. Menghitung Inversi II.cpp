...
void solve(){
    ...
    auto merge_sort = [&](auto self, int left, int right){
        if(left == right) return 0LL;
        int mid = (left + right) / 2;

        int ret = 0;
        ret += self(self, left, mid);
        ret += self(self, mid + 1, right);

        vec<int> temp;
        int i = left, j = mid + 1;
        while(i <= mid && j <= right){
            if(a[i] <= a[j]) temp.eb(a[i++]);
            else ret += (mid - i + 1), temp.eb(a[j++]);
        }
        while(i <= mid) temp.eb(a[i++]);
        while(j <= right) temp.eb(a[j++]);
        for(int i = 0; i < len(temp); i++) a[left + i] = temp[i];

        return ret;
    };

    cout << merge_sort(merge_sort, 0, n - 1);
}
