...
void solve(){
    int n; cin >> n;
    vec<pii> pesta(n);
    for(auto &[s, e] : pesta) cin >> s >> e;
    sort(all(pesta)); 
    
    vec<int> dp(n, -1);
    auto func = [&](auto func, int i) -> int {
        if(i == n) return 0;
        ...
        auto [si, ei] = pesta[i];
        // compute next interval first
        (res += func(func, i + 1)) %= MOD;

        // using binser to find the first index where the start time
        // is greater than to the end time of the current interval

        int next = n;
        for(int left = i + 1, right = n - 1; left <= right;){
            int mid = (left + right) / 2;
            auto [sj, ej] = pesta[mid];

            if(sj > ei) next = mid, right = mid - 1;
            else left = mid + 1;
        }

        (res += func(func, next)) %= MOD;
        return res;
    };
