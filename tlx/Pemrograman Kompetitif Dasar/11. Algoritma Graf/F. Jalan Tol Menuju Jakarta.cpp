...
void solve() {
  int n, m, t, a, b; cin >> n >> m >> t >> a >> b; a--, b--;
  vvvec<int> adj(n, 2);
  for(int u, v; m--;) cin >> u >> v, u--, v--, adj[u][0].eb(v), adj[v][0].eb(u);
  for(int u, v; t--;) cin >> u >> v, u--, v--, adj[u][1].eb(v), adj[v][1].eb(u);

  priority_queue<iii, vec<iii>, greater<iii>> pq;
  vvec<int> dist(n, 2, INF);
  dist[a][0] = 0;
  pq.emplace(0, a, 0);
  ...
  while(!pq.empty()) {
    ...
    if(u == b) return print(dist[u][used]);
    if(dist_u > dist[u][used]) continue;

    for(auto v : adj[u][0]) {
      if(dist[u][used] + 1 < dist[v][used]) {
