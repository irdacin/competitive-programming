...
void solve(){
  ...
  for(;;){
    ...
    for(int i = 0; i < r; i++){
      int cnt = 0;
      for(int j = 0; j < c; j++) cnt += grid[i][j] == '1';
    ...
    for(int j = 0; j < c; j++){
      int id = 0;
      int zero = 0, one = 0;
      for(; id < last; id++) zero += grid[id][j] == '0', one += grid[id][j] == '1';
      for(; id < r && grid[id][j] == '0'; id++) zero++;
      id--;

      // debug(j);
      // print(id, zero, one);

      for(; one-- > 0; id--) grid[id][j] = '1';
      for(; zero-- > 0; id--) grid[id][j] = '0';
    }
  }

  for(auto x : grid){
    for(auto i : x) prin(i); print();
  }
}
