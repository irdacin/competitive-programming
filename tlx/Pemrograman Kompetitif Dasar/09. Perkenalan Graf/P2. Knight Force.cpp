...
void solve() {
  ...
  vec<pii> dir = {{1, 2}, {2, 1}, {-1, 2}, {2, -1}, {1, -2}, {-2, 1}, {-1, -2}, {-2, -1}};
  queue<pii> q;
  vvec<int> dist(n, m, INF);
  for(int r, c; k--;) cin >> r >> c, --r, --c, q.emplace(r, c), dist[r][c] = 0;

  while(!q.empty()) {
    auto [row, col] = q.front(); q.pop();
    if(row + 1 == i && col + 1 == j) return print("TRUE");

    for(auto [x, y] : dir) {
      x += row, y += col;
      if(x >= 0 && x < n && y >= 0 && y < m && dist[x][y] > dist[row][col] + 1) {
        if(dist[row][col] + 1 > s) continue;
