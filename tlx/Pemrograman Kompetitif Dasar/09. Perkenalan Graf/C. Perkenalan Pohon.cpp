...
void solve() {
  int n, m; cin >> n >> m;
  map<string, vec<string>> adj[2];
  for(string u, v; m--;) cin >> u >> v, adj[0][u].eb(v), adj[1][v].eb(u);
  ...
  for(int i = 0; i < 2; i++) {
    ...
    while(!q.empty()) {
      ...
      if(u == a[i ^ 1]) {
        ...
        ans.eb(u);
        while(u != a[i]) {
          for(auto v : adj[1][u]) if(dist[v] + 1 == dist[u]) u = v;
          ans.eb(u);
        }

        for(auto x : ans | views::reverse) print(x);
        return;
      }
