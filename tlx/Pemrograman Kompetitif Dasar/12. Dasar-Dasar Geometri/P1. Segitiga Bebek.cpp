...
struct point {
  ...
  double dist(const point& p) const { return sqrtl((x - p.x) * (x - p.x) + (y - p.y) * (y - p.y)); }
...
void solve() {
  ...
  for(int i = 0; i < n; i++) {
    for(int j = i + 1; j < n; j++) {
      for(int k = j + 1; k < n; k++) {
        double luas = abs(bebek[i].cross(bebek[j], bebek[k])) / 2.0;
        if(luas < EPS) continue;

        if(luas < ans) ans = luas, cnt = 1;
        else if(abs(luas - ans) < EPS) cnt++;
      }
    }
  }
