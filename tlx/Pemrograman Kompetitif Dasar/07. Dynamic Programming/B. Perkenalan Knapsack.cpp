...
void solve(){
  ...
  for(int i = k - 1; i >= 0; i--){
    ...
    for(int capacity = 0; capacity <= n; capacity++){
      ...
      if(capacity >= w && dp[i][capacity] <= dp[i + 1][capacity - w] + h){
        dp[i][capacity] = dp[i + 1][capacity - w] + h;
      }
    }
  }

  // cari total beratnya sekecil mungkin
  int res = dp[0][n];
  for(; res == dp[0][n - 1]; n--);
  
  // utamakan batu dengan nomor lebih kecil
  for(int i = 0; i < k; i++){
    auto [w, h] = stone[i];
    if(n >= w && dp[i][n] == dp[i + 1][n - w] + h){
      print(i + 1);
      n -= w;
    }
  }
