...
void solve() {
  ...
  vvec<int> adj(n);
  for(int a, b; m--;) cin >> a >> b, --a, --b, adj[a].eb(b);

  // vvec<int> dp(n, k + 1);
  // auto func = [&](auto func, int cur, int cnt) {
  //   if(cnt == k && cur == n - 1) return 1LL;
  ...
  //   res = 0;
  //   for(auto v : adj[cur]) {
  //     if(cnt < k) res += func(func, v, cnt + 1);
  //   }

  //   return res;
  // };

  // print(func(func, 0, 0) % MOD);
  // dp[0][0] = 1;
  // for(int j = 0; j < k; j++) {
  // for(int i = 0; i < n; i++) {
  //   for(auto v : adj[i]) {
  ...
  matrix mat(n, n);
  for(int i = 0; i < n; i++) for(auto v : adj[i]) mat[i][v] += 1;
  print(pow(mat, k)[0][n - 1]);
}
