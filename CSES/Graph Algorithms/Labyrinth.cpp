...
void solve(){
    ...
    pii start = {0, 0};
    for(bool flag = false; start.fi < n && !flag; start.fi++) for(start.se = 0; start.se < m && !flag; start.se++) if(grid[start.fi][start.se] == 'A') flag = true;
    --start.fi, --start.se;

    vec<tuple<int, int, char>> dir = {{0, -1, 'R'}, {-1, 0, 'D'}, {0, 1, 'L'}, {1, 0, 'U'}};
    ...
    while(!q.empty()){
        ...
        if(grid[row][col] == 'B'){
            ...
            for(; grid[row][col] != 'A';){
                for(auto [x, y, ch] : dir){
                    x += row, y += col;
                    if(x >= 0 && x < n && y >= 0 && y < m && dist[x][y] == dist[row][col] - 1){
                        path.eb(ch);
                        row = x, col = y;
                        break;
                    }
        ...
        for(auto [x, y, ch] : dir){
            ...
            if(x >= 0 && x < n && y >= 0 && y < m && (grid[x][y] == '.' || grid[x][y] == 'B') && dist[x][y] > dist[row][col] + 1){
