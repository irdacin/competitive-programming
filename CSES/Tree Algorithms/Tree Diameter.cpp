...
void solve(){
	int n; cin >> n;
	vvec<int> edge(n);
	for(int a, b; n --> 1;) cin >> a >> b, --a, --b, edge[a].eb(b), edge[b].eb(a);

	auto dfs = [&](auto dfs, int cur, int prev, int dis) -> pii {
		int mxDistance = dis, farthest = cur;
		for(auto nxt : edge[cur]){
			if(nxt != prev){
				auto [d, x] = dfs(dfs, nxt, cur, dis + 1);
				if(d > mxDistance){
					mxDistance = d;
					farthest = x;
				}
		...
		return {mxDistance, farthest};
	};
	auto [dis, u] = dfs(dfs, 0, -1, 0);
	cout << dfs(dfs, u, -1, 0).fi;
}
