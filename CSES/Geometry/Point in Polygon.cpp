...
inline void print(T&& A, U&&... B) { cout << A; if (sizeof...(U)) cout << ' '; print(B...); }
...
void solve() {
  ...
  for(point a; m--;) {
    ...
    [&]() {
      ...
      for(int i = 0; i < n; i++) {
        point p = polygon[i];
        point q = i + 1 < n ? polygon[i + 1] : polygon[0];

        if(intersect(p, q, a, a)) return print("BOUNDARY");
        if(p.y <= a.y && q.y > a.y && a.cross(p, q) > 0) winding++;
        else if(p.y > a.y && q.y <= a.y && a.cross(p, q) < 0) winding--;
      }

      print(winding != 0 ? "INSIDE" : "OUTSIDE");
    }();
  }
}
