...
void solve() {
  ...
  for(int sz = 1; sz <= len(s); sz++) {

    bool flag = true;
    for(int i = 0; i < len(s); i += sz) {

        // print(i, i + sz);
        // print(rh.get(i, i + sz + 1));
        // print(rh.get(0,  sz + 1));
        // print(s.substr(i, sz));
        // print(s.substr(0, sz ));
      if(i + sz < len(s)) {
        if(rh.get(i, i + sz + 1) != rh.get(0, sz + 1)) flag = false;
      } else {
        if(rh.get(i, len(s)) != rh.get(0, len(s) - i)) flag = false;
      }
    }
    // print();
    if(flag) prinsp(sz);
