...
struct SegmentTree {
  ...
  void update(int root, int pos, node t) { update(root, l0, r0, pos, t); } // [pos]
...
void solve() {
  ...
  SegmentTree<node> seg(6'000'001);
  int root = seg.new_root(0, 1e9);
  for(auto &x : p) x--, seg.update(root, x, node(1));

  for(char c; q--;) {
    cin >> c;
    if(int k, x, a, b; c == '!') {
      cin >> k >> x; k--; x--;
      seg.update(root, p[k], node(-1));
      seg.update(root, x, node(1));
      p[k] = x;
    } else cin >> a >> b, a--, print(seg.query(root, a, b).val);
  }
}
