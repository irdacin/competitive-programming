...
struct SegmentTree {
	...
	SegmentTree(vec<int> a) : n(len(a)), v(a), tree(4 * n) {
		build(0, n - 1);
	}

	void build(int left, int right, int index = 0){
		if(left == right){
			tree[index] = v[left];
			return;
		}
	...
	int query(int val){
		return query(0, n - 1, 0, val) + 1;
	}

	int query(int left, int right, int index, int val){
		if(left == right){
            if(tree[index] >= val){
                tree[index] -= val;
                return left;
		...
		int mid = (left + right) / 2;
		int res = -1;
		if(tree[2 * index + 1] >= val) res = query(left, mid, 2 * index + 1, val);
		if(res == -1 && tree[2 * index + 2] >= val) res = query(mid + 1, right, 2 * index + 2, val);
		tree[index] = max(tree[2 * index + 1], tree[2 * index + 2]);
		return res;
	}
};
...
void solve(){
	...
	for(int r; m--;) cout << (cin >> r, seg.query(r)) << ' ';
}
